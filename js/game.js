...
function create() {
    ...
    star = game.add.sprite(170, 79, 'star');
    game.physics.arcade.enable(star);    //星星物理引擎
    star.body.immovable = true;          //星星不可被推擠對嗎? 對
    many_star.push(star);


    orin = game.add.sprite(170, 100, 'normal');  //初始站立平台
    game.physics.arcade.enable(orin);   //物理引擎
    orin.body.immovable = true;         //不可被推擠對嗎? 對
    orin.height = 20;
    orin.width = 60;

    var gamerules = game.add.image(100, 30, 'gamerules');
    button = game.add.button(181, 370, 'button', actionOnClick, this, 2, 1, 0);  //建按鈕
    ...
    var style = { fill: '#ff0000', fontSize: '30px' } //設定style屬性
    var styleII = { fill: '#ff0000', fontSize: '20px' } //設定style屬性
    var styleIII = { fill: 'yellow', fontSize: '30px' } //設定style屬性

    playerdied = game.add.sprite(225, 140, 'playerdied');
    ...
    text_over = game.add.text(168, 250, '點擊畫面重新開始', styleIII);  //添加死亡後跳出的字
    text_over.visible = false;   //跳出死亡的字遊戲剛開始顯示是false等真的死後會變true跳出

    text_point = game.add.text(475, 25, '', style);
    text_Bonus = game.add.text(440, 60, '', styleII);
...
function createPlayer() {
    player = game.add.sprite(200, 31, 'player');
...
function createPlatform() {         //創磚塊
    ...
    var rand = Math.random() * (550 - 120 - 34 - 33) + 33;  //Math.random()是0~0.9999 乘500=0~500,139是格寬,34是兩個牆寬,33是人寬,最後加33是為了最小數可以是33
...
function creatBounders() {                             //創牆跟頂刺
    ...
    rightWall = game.add.sprite(533, 0, 'wall');//創右牆
